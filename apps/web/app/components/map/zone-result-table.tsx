  ...
  return (
    <>
      <div className="map-metrics">
        ...
        <MetricCard label="평균 사용승인년도" value={summary.average_approval_year ? String(summary.average_approval_year) : "-"} />
        ...
        <MetricCard label="평균 용적률(%)" value={formatNumber(summary.average_floor_area_ratio)} />
      ...
      <p className="hint">건축 지표 기준: 노후도는 사용승인 30년 이상, 과소필지는 150㎡ 미만 필지를 기준으로 계산합니다.</p>
      ...
      <table className="data-table map-zone-table">
        <thead>
          <tr>
            ...
            <th className="center">건축물 수</th>
            <th className="center">노후 건물 수</th>
            <th className="center">평균 사용승인년도</th>
          ...
          {parcels.map((row) => {
            ...
            return (
              <tr key={row.pnu} className={!row.included ? "excluded" : ""} onClick={() => onLocate(row.lat, row.lng)}>
                ...
                <td className="center">{row.primary_purpose_name || "-"}</td>
                <td className="center">{formatNumber(row.building_count)}</td>
                <td className="center">{formatNumber(row.aged_building_count)}</td>
                <td className="center">{row.average_approval_year || "-"}</td>
                <td className="right">{formatNumber(row.floor_area_ratio)}</td>
