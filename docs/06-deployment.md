...
- 이전 안정 태그: `v2.2.0`
- 현재 문서 기준 범위:
  - Web v3 안정화 단계 (개별/지도/파일/이용내역/마이페이지/정책 페이지)
  - API v3 안정화 단계 (인증 확장 + 지도/구역조회 + 파일조회 + 기록 + AI 보조 계층)
  - Android Wrapper(APK/AAB 배포)
  - 구역조회 미리보기/명시 저장/비교/리뷰 워크플로우
...
2. 개별조회: 지번/도로명 조회
3. 지도조회: 지도 렌더링/클릭/주소검색/CSV/상세조회/구역 분석/리뷰 큐/비교
4. 파일조회: 업로드/진행률/다운로드/삭제/실패 원인 요약
